...
namespace Drupal\graphql_compose_extra_edges\Plugin\GraphQL\SchemaExtension;
 ...
 *
 * @SchemaExtension(
 *   id = "graphql_compose_extra_edges_schema",
...
class EdgesSchemaExtension extends SdlSchemaExtensionPluginBase
{
  ...
  public function registerResolvers(ResolverRegistryInterface $registry)
  {
    ...
    foreach ($this->getEnabledBundlePlugins() as $bundle) {
      // graphql_compose_extra_edges_entity_type:node:page.
      $default_producer = 'graphql_compose_extra_edges_entity_type:' . $bundle->entityTypePlugin->getPluginId() . ':' . $bundle->entity->id();
    ...
    $registry->addFieldResolver(
      'Query',
      'nodes',
      $builder->produce('graphql_compose_extra_edges_node')
        ->map('after', $builder->fromArgument('after'))
        ->map('before', $builder->fromArgument('before'))
        ->map('first', $builder->fromArgument('first'))
        ->map('last', $builder->fromArgument('last'))
