...
use Drupal\graphql_compose_extra_edges\Wrappers\EdgeInterface;
use Drupal\graphql_compose_extra_edges\ConnectionInterface;
...
class EntityConnection implements ConnectionInterface
{
  ...
  {
    $search_query = $this->queryHelper->getSearchQuery();
    ...
    if (!is_null($cursor)) {
      ...
      $pagination_condition = $search_query->createConditionGroup('OR');
      $pagination_condition->addCondition($sort_field, $cursor_sort, $operator);
      ...
      if ($sort_field !== $id_field) {
        $pagination_condition->addConditionGroup(
          $search_query->createConditionGroup('AND')
            ->addCondition($sort_field, $cursor_sort, '=')
            ->addCondition($id_field, $cursor_id, $operator)
        );
      }

      $search_query->addConditionGroup($pagination_condition);
    }
    ...
    if (!is_null($langcode)) {
      ...
      if (!is_null($langcode_field)) {
        $search_query->setLanguages([$langcode]);
      }
    }
    ...
    $search_query->range(0, $limit + 1);

    $search_query->sort('search_api_relevance', 'DESC');
    ...
    if ($published && $account) {
      ...
      if (!array_filter($bypass, $account->hasPermission(...))) {
        $search_query->addCondition($published, TRUE);
      }
    }

    // Fetch the search results.
    $search_result = $this->applySearch($search_query);

    return $this->queryHelper->getLoaderPromise($search_result);
  }
  ...
  protected function applySearch(QueryInterface $query)
  {
    ...
    foreach ($execute as $item) {
      ...
      foreach ($data as $matches) {
        foreach ($matches as $match) {
          $removeTags = str_replace(array("<strong>", "</strong>"), "", $match);
          $words = explode(" ", $removeTags);
          $arr = [];
          foreach ($words as $value) {
            if (preg_match("|ing|", $value)) {
              $arr[] = $value;
            }
          }
        }
    ...
    $results = $execute->getResultItems();
